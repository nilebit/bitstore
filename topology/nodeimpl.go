...
type Node interface {
	...
	UpAdjustMaxVolumeId(vid util.VIDType)

	GetVolumeCount() int
	GetActiveVolumeCount() int
	GetMaxVolumeCount() int
	GetMaxVolumeId() util.VIDType
	SetParent(Node)
...
func (n *NodeImpl) UpAdjustMaxVolumeId(vid util.VIDType) { //can be negative
...
func (n *NodeImpl) GetMaxVolumeId() util.VIDType {
